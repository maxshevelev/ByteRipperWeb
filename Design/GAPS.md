...
| G38 | **The drag dead zone.** A click that lands between a byte's nibbles — from the middle of the high-nibble character to the middle of the low-nibble one (§3.3) — is a caret position, and the drag boundary sits on the byte's centre, so a 1 px jitter there selects the byte by accident. `HexView.dragHasLeftDeadZone(from:to:)` holds the selection back until the pointer leaves that zone, and a click outside it — a byte's outer quarters, the offset or ASCII columns — engages at once. The web's drag extends on every pointer move, with no zone. G19 left the geometry behind it (`HexLayout.highNibbleMidX` / `lowNibbleMidX`). | `HexView.swift` (private) | 3 | — | P3 | open |
