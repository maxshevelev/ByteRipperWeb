 ...
 *
 * `parseEfs` is the byte-derived structural decode: the page inventory, the
 * System page fields, the index permutation and the CRCs of the page headers,
 * index areas and Data page footers. The file walk that follows it (`efsDataArea`
 * + `efsFiles`) needs the external `FileTable.dat`: an EFS volume's pages are one
 * flat byte area, and the offsets that cut it into files are the EFST records,
 * with the Integrity flag that decides each file's end coming from the FTBL rows
 * beside them — which is why upstream calls that read necessary and not optional
 * (MEA.py 8846). Reading the FITC's config records needs the same table.
 *
...
export function efsDataArea(
  ...
  if (!permutation) return new Uint8Array(0);
  const pageBytes = PAGE_SIZE - PAGE_HEADER_SIZE - FOOTER_SIZE;
  const area = new Uint8Array(bases.length * pageBytes);
  order.forEach((value, at) => {
    const base = bases[value] ?? 0;
    area.set(
      buffer.subarray(base + PAGE_HEADER_SIZE, base + PAGE_SIZE - FOOTER_SIZE),
      at * pageBytes
    );
  });
  return area;
 ...
 */
function efsDataPageBases(buffer: Uint8Array): number[] {
  const bases: number[] = [];
...
/**
 * `EFS_Page_Footer`: the eight bytes a Data page ends with, which the data area
...
export function efsFiles(options: {
  ...
  readonly entries: readonly FileTableEFSEntry[];
  ...
  for (const entry of [...entries].sort((a, b) => a.dataOffset - b.dataOffset)) {
    ...
    if (storedSize === undefined || unknown === undefined) continue;
    if (storedSize === 0xffff) continue;
