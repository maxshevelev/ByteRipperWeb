...
export function parseMfs(
  ...
  // (`configRecordSize` — 0x1C on CSME 11/12 and their analogues, 0xC on CSME
  // 13–16), and this parser knows only bytes. The decode is `configurations`
  // below, run from the analyzer's identity-gated phase over the files retained
  // here — the same place the Home Directory and the Integrity split are
  // decoded.
  return info;
}
 ...
 * `MFS_Config_Record_0xC` entries. Bounded the same way as the 0x1C walk — the
 * declared count leads, and a stream shorter than the table it declares yields
 * the complete records that fit.
 *
 ...
 * variant/major/minor and `platform` (`vol_ftbl_pl`); the hotfix is unused
...
/**
 * The volume's Intel (6) and OEM (7) Configuration record streams, read with
 * the record struct this identity uses (upstream `mfs_cfg_anl` over
...
}): { readonly byName: MFSConfigDecode[]; readonly byId: MFSConfigIdDecode[] } {
 ...
 * `protectedIndices` is handed in by the caller, which is the one place that
 * has the table, and this stays a byte decode: what comes out is the tail's own
 ...
 * FTBL or the volume to say which. Upstream's workaround — "stupid AF but
 * should work until the proper indicator can be found" — is to read the table,
 * and if its Anti-Replay counter comes out absurdly large (> 0xFFFF), re-split
 * 0x10 further back. It is ported as written, because a wrong split is a wrong
 * file: on `CSME 15.bin` it is the difference between 233 files that end at
 * 0x28 and 132 that end at 0x38.
 *
...
export function ftblFileIntegrity(options: {
  ...
  for (const file of files) {
    ...
    if (sec === 0x28 && table.arCounter > 0xffff && file.content.length >= 0x38) {
      ...
      if (wider !== undefined) {
        // The table sits 0x10 earlier than it looked: re-read it there and
        // count the extra bytes as part of what the file ends with.
        size = 0x38;
        table = wider;
      }
    }
  ...
  return result.sort((one, other) => one.fileIndex - other.fileIndex);
}
