/**
 * SHA-1, SHA-256 and SHA-384, synchronous and without a platform API.
 *
 * The browser has `crypto.subtle`, and it is asynchronous. The ME parser is
 * written against synchronous reads for the reason the whole firmware half is —
 * a worker's `FileReaderSync` lets six thousand lines of parser port as they
 * stand — and a digest that had to be awaited would turn every signature check
 * inside out into continuations. `src/firmware` is also compiled without the DOM
 * (D1), so `crypto` is not in scope here at all.
 *
 * Written out rather than taken from a package, which is a decision worth its
 * reason: these three are completely specified by FIPS 180-4, and NIST publishes
 * the vectors below. A wrong implementation fails them on the first run, so
 * correctness here is *checked* rather than trusted — which is the only
 * condition under which hand-writing a hash is the better choice, since what it
 * decides is whether the application tells a bench that a firmware signature is
 * valid.
 *
 * Ported alongside `Packages/MEFirmware/Crypto`, whose `Digest` is CryptoKit.
 */

/**
 * Uppercase hex — the display form upstream uses for a digest.
 *
 * @upstream Packages/MEFirmware/Sources/MEFirmware/Crypto/Digest.swift#Digest.sha256Hex
 * @upstream Packages/MEFirmware/Sources/MEFirmware/Crypto/Digest.swift#Digest.sha384Hex
 * @upstream Packages/MEFirmware/Sources/MEFirmware/Crypto/RSA.swift#Digest.hex
 * @upstream-differs hex(sha256(bytes)) at the call site, rather than a hex variant of each digest
 */
export function hex(bytes: Uint8Array): string {
  let text = "";
  for (const byte of bytes) text += byte.toString(16).toUpperCase().padStart(2, "0");
  return text;
}

/** Whether two digests are the same bytes. */
export function sameBytes(left: Uint8Array, right: Uint8Array): boolean {
  if (left.length !== right.length) return false;
  for (let index = 0; index < left.length; index++) {
    if (left[index] !== right[index]) return false;
  }
  return true;
}

// MARK: - The 32-bit family: SHA-1 and SHA-256

/**
 * The message with its padding: the `0x80` marker, zeros, and the bit length as
 * a 64-bit big-endian number. Both 32-bit digests below pad the same way.
 */
function padded32(message: Uint8Array): Uint8Array {
  const withMarker = message.length + 1;
  const blocks = Math.ceil((withMarker + 8) / 64);
  const out = new Uint8Array(blocks * 64);
  out.set(message);
  out[message.length] = 0x80;
  // The length in bits. A message past 2^53 bits is past what this application
  // can hold anyway — a flash dump is gigabytes — so the high word is written
  // from the same number rather than from a separate counter.
  const bits = message.length * 8;
  const view = new DataView(out.buffer);
  view.setUint32(out.length - 8, Math.floor(bits / 2 ** 32), false);
  view.setUint32(out.length - 4, bits >>> 0, false);
  return out;
}

const rotl32 = (value: number, by: number) => ((value << by) | (value >>> (32 - by))) >>> 0;
const rotr32 = (value: number, by: number) => ((value >>> by) | (value << (32 - by))) >>> 0;

/**
 * @upstream Packages/MEFirmware/Sources/MEFirmware/Crypto/Digest.swift#Digest
 * @upstream Packages/MEFirmware/Sources/MEFirmware/Crypto/Digest.swift#Digest.sha1
 */
export function sha1(message: Uint8Array): Uint8Array {
  const data = padded32(message);
  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  let [a0, b0, c0, d0, e0] = [0x6745_2301, 0xefcd_ab89, 0x98ba_dcfe, 0x1032_5476, 0xc3d2_e1f0];
  const w = new Uint32Array(80);

  for (let at = 0; at < data.length; at += 64) {
    for (let index = 0; index < 16; index++) w[index] = view.getUint32(at + index * 4, false);
    for (let index = 16; index < 80; index++) {
      w[index] = rotl32(
        ((w[index - 3] ?? 0) ^
          (w[index - 8] ?? 0) ^
          (w[index - 14] ?? 0) ^
          (w[index - 16] ?? 0)) >>>
          0,
        1
      );
    }

    let [a, b, c, d, e] = [a0, b0, c0, d0, e0];
    for (let index = 0; index < 80; index++) {
      let f: number;
      let k: number;
      if (index < 20) {
        f = (b & c) | (~b & d);
        k = 0x5a82_7999;
      } else if (index < 40) {
        f = b ^ c ^ d;
        k = 0x6ed9_eba1;
      } else if (index < 60) {
        f = (b & c) | (b & d) | (c & d);
        k = 0x8f1b_bcdc;
      } else {
        f = b ^ c ^ d;
        k = 0xca62_c1d6;
      }
      const next = (rotl32(a, 5) + (f >>> 0) + e + k + (w[index] ?? 0)) >>> 0;
      e = d;
      d = c;
      c = rotl32(b, 30);
      b = a;
      a = next;
    }
    a0 = (a0 + a) >>> 0;
    b0 = (b0 + b) >>> 0;
    c0 = (c0 + c) >>> 0;
    d0 = (d0 + d) >>> 0;
    e0 = (e0 + e) >>> 0;
  }

  const out = new Uint8Array(20);
  const result = new DataView(out.buffer);
  for (const [index, word] of [a0, b0, c0, d0, e0].entries())
    result.setUint32(index * 4, word, false);
  return out;
}

/** SHA-256's round constants: the cube roots of the first sixty-four primes. */
const K256 = new Uint32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);

/** @upstream Packages/MEFirmware/Sources/MEFirmware/Crypto/Digest.swift#Digest.sha256 */
export function sha256(message: Uint8Array): Uint8Array {
  const data = padded32(message);
  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  const h = Uint32Array.of(
    0x6a09e667,
    0xbb67ae85,
    0x3c6ef372,
    0xa54ff53a,
    0x510e527f,
    0x9b05688c,
    0x1f83d9ab,
    0x5be0cd19
  );
  const w = new Uint32Array(64);

  for (let at = 0; at < data.length; at += 64) {
    for (let index = 0; index < 16; index++) w[index] = view.getUint32(at + index * 4, false);
    for (let index = 16; index < 64; index++) {
      const x = w[index - 15] ?? 0;
      const y = w[index - 2] ?? 0;
      const s0 = (rotr32(x, 7) ^ rotr32(x, 18) ^ (x >>> 3)) >>> 0;
      const s1 = (rotr32(y, 17) ^ rotr32(y, 19) ^ (y >>> 10)) >>> 0;
      w[index] = ((w[index - 16] ?? 0) + s0 + (w[index - 7] ?? 0) + s1) >>> 0;
    }

    let [a, b, c, d, e, f, g, hh] = h;
    for (let index = 0; index < 64; index++) {
      const s1 = (rotr32(e ?? 0, 6) ^ rotr32(e ?? 0, 11) ^ rotr32(e ?? 0, 25)) >>> 0;
      const ch = (((e ?? 0) & (f ?? 0)) ^ (~(e ?? 0) & (g ?? 0))) >>> 0;
      const t1 = ((hh ?? 0) + s1 + ch + (K256[index] ?? 0) + (w[index] ?? 0)) >>> 0;
      const s0 = (rotr32(a ?? 0, 2) ^ rotr32(a ?? 0, 13) ^ rotr32(a ?? 0, 22)) >>> 0;
      const maj = (((a ?? 0) & (b ?? 0)) ^ ((a ?? 0) & (c ?? 0)) ^ ((b ?? 0) & (c ?? 0))) >>> 0;
      const t2 = (s0 + maj) >>> 0;
      hh = g;
      g = f;
      f = e;
      e = ((d ?? 0) + t1) >>> 0;
      d = c;
      c = b;
      b = a;
      a = (t1 + t2) >>> 0;
    }
    for (const [index, value] of [a, b, c, d, e, f, g, hh].entries()) {
      h[index] = ((h[index] ?? 0) + (value ?? 0)) >>> 0;
    }
  }

  const out = new Uint8Array(32);
  const result = new DataView(out.buffer);
  for (let index = 0; index < 8; index++) result.setUint32(index * 4, h[index] ?? 0, false);
  return out;
}

// MARK: - The 64-bit family: SHA-384

/**
 * SHA-384's round constants, as `bigint`.
 *
 * The 64-bit family is written with `bigint` rather than with pairs of 32-bit
 * words. It is the slower of the two, and deliberately: what runs through here
 * is a manifest's protected window — a few kilobytes, once per manifest — where
 * the 32-bit digests above run over module bodies and whole regions. Paying for
 * legibility on the path that is not hot is the right way round, and the pair
 * arithmetic is where a hand-written SHA-512 goes wrong.
 */
const K512: readonly bigint[] = [
  0x428a2f98d728ae22n,
  0x7137449123ef65cdn,
  0xb5c0fbcfec4d3b2fn,
  0xe9b5dba58189dbbcn,
  0x3956c25bf348b538n,
  0x59f111f1b605d019n,
  0x923f82a4af194f9bn,
  0xab1c5ed5da6d8118n,
  0xd807aa98a3030242n,
  0x12835b0145706fben,
  0x243185be4ee4b28cn,
  0x550c7dc3d5ffb4e2n,
  0x72be5d74f27b896fn,
  0x80deb1fe3b1696b1n,
  0x9bdc06a725c71235n,
  0xc19bf174cf692694n,
  0xe49b69c19ef14ad2n,
  0xefbe4786384f25e3n,
  0x0fc19dc68b8cd5b5n,
  0x240ca1cc77ac9c65n,
  0x2de92c6f592b0275n,
  0x4a7484aa6ea6e483n,
  0x5cb0a9dcbd41fbd4n,
  0x76f988da831153b5n,
  0x983e5152ee66dfabn,
  0xa831c66d2db43210n,
  0xb00327c898fb213fn,
  0xbf597fc7beef0ee4n,
  0xc6e00bf33da88fc2n,
  0xd5a79147930aa725n,
  0x06ca6351e003826fn,
  0x142929670a0e6e70n,
  0x27b70a8546d22ffcn,
  0x2e1b21385c26c926n,
  0x4d2c6dfc5ac42aedn,
  0x53380d139d95b3dfn,
  0x650a73548baf63den,
  0x766a0abb3c77b2a8n,
  0x81c2c92e47edaee6n,
  0x92722c851482353bn,
  0xa2bfe8a14cf10364n,
  0xa81a664bbc423001n,
  0xc24b8b70d0f89791n,
  0xc76c51a30654be30n,
  0xd192e819d6ef5218n,
  0xd69906245565a910n,
  0xf40e35855771202an,
  0x106aa07032bbd1b8n,
  0x19a4c116b8d2d0c8n,
  0x1e376c085141ab53n,
  0x2748774cdf8eeb99n,
  0x34b0bcb5e19b48a8n,
  0x391c0cb3c5c95a63n,
  0x4ed8aa4ae3418acbn,
  0x5b9cca4f7763e373n,
  0x682e6ff3d6b2b8a3n,
  0x748f82ee5defb2fcn,
  0x78a5636f43172f60n,
  0x84c87814a1f0ab72n,
  0x8cc702081a6439ecn,
  0x90befffa23631e28n,
  0xa4506cebde82bde9n,
  0xbef9a3f7b2c67915n,
  0xc67178f2e372532bn,
  0xca273eceea26619cn,
  0xd186b8c721c0c207n,
  0xeada7dd6cde0eb1en,
  0xf57d4f7fee6ed178n,
  0x06f067aa72176fban,
  0x0a637dc5a2c898a6n,
  0x113f9804bef90daen,
  0x1b710b35131c471bn,
  0x28db77f523047d84n,
  0x32caab7b40c72493n,
  0x3c9ebe0a15c9bebcn,
  0x431d67c49c100d4cn,
  0x4cc5d4becb3e42b6n,
  0x597f299cfc657e2an,
  0x5fcb6fab3ad6faecn,
  0x6c44198c4a475817n,
];

const MASK64 = 0xffff_ffff_ffff_ffffn;
const rotr64 = (value: bigint, by: bigint) =>
  ((value >> by) | ((value << (64n - by)) & MASK64)) & MASK64;

/**
 * SHA-512: the same rounds as SHA-384 over a state of its own, and none of it
 * thrown away.
 *
 * @web-only upstream reaches CryptoKit's `SHA512` for a Boot Guard digest stored
 * with that algorithm id; here the 64-bit family is written out once and both
 * lengths are the same function.
 */
export function sha512(message: Uint8Array): Uint8Array {
  return sha512Family(message, [
    0x6a09e667f3bcc908n,
    0xbb67ae8584caa73bn,
    0x3c6ef372fe94f82bn,
    0xa54ff53a5f1d36f1n,
    0x510e527fade682d1n,
    0x9b05688c2b3e6c1fn,
    0x1f83d9abfb41bd6bn,
    0x5be0cd19137e2179n,
  ]).subarray(0, 64);
}

/** @upstream Packages/MEFirmware/Sources/MEFirmware/Crypto/Digest.swift#Digest.sha384 */
export function sha384(message: Uint8Array): Uint8Array {
  // SHA-384's initial state: the fractional parts of the square roots of the
  // ninth through sixteenth primes, which is what makes it more than a
  // truncated SHA-512.
  return sha512Family(message, [
    0xcbbb9d5dc1059ed8n,
    0x629a292a367cd507n,
    0x9159015a3070dd17n,
    0x152fecd8f70e5939n,
    0x67332667ffc00b31n,
    0x8eb44a8768581511n,
    0xdb0c2e0d64f98fa7n,
    0x47b5481dbefa4fa4n,
  ]).subarray(0, 48);
}

/** The 64-bit family's rounds: SHA-512 and SHA-384 differ only in where they start. */
function sha512Family(message: Uint8Array, initial: readonly bigint[]): Uint8Array {
  // The 64-bit family pads to 128 bytes with a 128-bit length. The high half of
  // that length is always zero here: it would need a message of 2^64 bits.
  const withMarker = message.length + 1;
  const blocks = Math.ceil((withMarker + 16) / 128);
  const data = new Uint8Array(blocks * 128);
  data.set(message);
  data[message.length] = 0x80;
  const view = new DataView(data.buffer);
  const bits = BigInt(message.length) * 8n;
  view.setBigUint64(data.length - 16, (bits >> 64n) & MASK64, false);
  view.setBigUint64(data.length - 8, bits & MASK64, false);

  const h = [...initial];
  const w = new Array<bigint>(80);

  for (let at = 0; at < data.length; at += 128) {
    for (let index = 0; index < 16; index++) w[index] = view.getBigUint64(at + index * 8, false);
    for (let index = 16; index < 80; index++) {
      const x = w[index - 15] ?? 0n;
      const y = w[index - 2] ?? 0n;
      const s0 = rotr64(x, 1n) ^ rotr64(x, 8n) ^ (x >> 7n);
      const s1 = rotr64(y, 19n) ^ rotr64(y, 61n) ^ (y >> 6n);
      w[index] = ((w[index - 16] ?? 0n) + s0 + (w[index - 7] ?? 0n) + s1) & MASK64;
    }

    let [a, b, c, d, e, f, g, hh] = h as [
      bigint,
      bigint,
      bigint,
      bigint,
      bigint,
      bigint,
      bigint,
      bigint,
    ];
    for (let index = 0; index < 80; index++) {
      const s1 = rotr64(e, 14n) ^ rotr64(e, 18n) ^ rotr64(e, 41n);
      const ch = (e & f) ^ (~e & MASK64 & g);
      const t1 = (hh + s1 + ch + (K512[index] ?? 0n) + (w[index] ?? 0n)) & MASK64;
      const s0 = rotr64(a, 28n) ^ rotr64(a, 34n) ^ rotr64(a, 39n);
      const maj = (a & b) ^ (a & c) ^ (b & c);
      const t2 = (s0 + maj) & MASK64;
      hh = g;
      g = f;
      f = e;
      e = (d + t1) & MASK64;
      d = c;
      c = b;
      b = a;
      a = (t1 + t2) & MASK64;
    }
    for (const [index, value] of [a, b, c, d, e, f, g, hh].entries()) {
      h[index] = ((h[index] ?? 0n) + value) & MASK64;
    }
  }

  // The whole state; SHA-384 is this truncated to its first six words.
  const out = new Uint8Array(64);
  const result = new DataView(out.buffer);
  for (let index = 0; index < 8; index++) result.setBigUint64(index * 8, h[index] ?? 0n, false);
  return out;
}
