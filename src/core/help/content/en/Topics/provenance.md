...
- **Bench lore, marked as such.** Some of what the glossary says is not decode at all: where a vendor tends to hide a serial number, what an embedded controller's firmware usually starts with, which NVRAM variable holds a password. That comes from repair communities — the [[web:https://github.com/ISpillMyDrink/UEFI-Repair-Guide/wiki|UEFI Repair Guide]] wiki among them — and no datasheet backs it. The help says so wherever it is used: those are signs to check against a known-good dump, never a diagnosis on their own. Where a page leans on one of these, it links to it, so that a claim nobody can look up in a datasheet can at least be traced to whoever measured it.
