...
/** @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.defaultChunkSize */
export const DEFAULT_SEARCH_CHUNK_SIZE = 1024 * 1024;
/**
 * Past this many, a results list stops being a tool and starts being a wall.
 *
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.defaultMaxResults
 */
export const DEFAULT_MAX_RESULTS = 1000;

/**
 * A search asked for no bytes at all: nothing to look for is a mistake in the
 * caller, not an answer of "nowhere".
 *
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchError.emptyPattern
 */
export class SearchPatternEmpty extends Error {
  constructor() {
    super("The pattern is empty.");
    this.name = "SearchPatternEmpty";
  }
}
 ...
 *
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.foldByte
 */
 ...
 *
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.foldInPlace
 */
 ...
 *
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.firstMatch
 */
 ...
 *
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.lastMatch
 */
...
export interface ScanOptions {
  ...
  /**
   * Called between windows; a promise it returns is awaited before the next.
   *
   * A scan on the main thread passes one that hands the event loop a turn once
   * a slice is spent — reads the cache answers resolve as microtasks, and a loop
   * of those otherwise holds the interface until the file is done.
   *
   * @web-only upstream scans on a background queue and never shares its thread with the interface
   */
  readonly pause?: () => Promise<void> | undefined;
}

/** Waits out the caller's pause, if it asks for one. */
async function takeTurn(options: ScanOptions): Promise<void> {
 ...
 *
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.find
 */
export async function findOne(
  ...
  options: {
    ...
    /** @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchDirection */
  ...
  if (length === 0) throw new SearchPatternEmpty();
  ...
  if (!backward) {
    ...
    for (let cursor = Math.min(from, limit); cursor < limit; cursor += chunkSize) {
      ...
      // Windows overlap by `length - 1`, so summing their lengths overshoots the
      // file; the ground actually covered is what is reported.
      if (size > 0) options.onProgress?.(Math.min(cursor + chunkSize, size) / size);
      await takeTurn(options);
    }
  ...
  // Progress is measured over the region actually being searched — [0, from) —
  // not the whole file: a backward search from a caret near the start otherwise
  // opened at 91 % and crawled to 100 %.
  const span = from;
  for (let end = from; end > 0; end -= chunkSize) {
    ...
    if (at !== -1) return { start: cursor + at, end: cursor + at + length };
    if (span > 0) options.onProgress?.((span - Math.max(0, end - chunkSize)) / span);
    await takeTurn(options);
  }
  if (span > 0) options.onProgress?.(1);
  return undefined;
}
 ...
 *
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.findAll
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.findAllStream
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.matchStartsStream
 * @upstream Packages/ByteRipperCore/Sources/ByteRipperCore/SearchEngine.swift#SearchEngine.MatchBatch
 * @upstream-differs one scan handing each window's match starts and its covered extent to callbacks, instead of an array, a stream and a batch stream
 */
